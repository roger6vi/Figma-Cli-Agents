...
// Hot-reload FigmaClient using a cache-busting query instead of temp files in src/
const __dirname = dirname(fileURLToPath(import.meta.url));
const figmaClientPath = join(__dirname, 'figma-client.js');
const figmaClientUrl = pathToFileURL(figmaClientPath);
...
async function getFigmaClient() {
  try {
    ...
    if (!FigmaClient || modTime > lastModTime) {
      const module = await import(`${figmaClientUrl.href}?mtime=${encodeURIComponent(String(modTime))}`);
    ...
    if (!FigmaClient) {
      const module = await import(figmaClientUrl.href);
      FigmaClient = module.FigmaClient;
    }
  }
  return FigmaClient;
...
const TOKEN_FILE = join(tmpdir(), '.figma-daemon-token');
