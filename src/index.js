...
const DAEMON_PID_FILE = join(TMP_ROOT, '.figma-cli-daemon.pid');
const DAEMON_TOKEN_FILE = join(TMP_ROOT, '.figma-daemon-token');
...
const PLUGIN_MANIFEST_PATH = join(__dirname, '..', 'plugin', 'manifest.json');
...
function loadConfig() {
  try {
    ...
    if (existsSync(LEGACY_CONFIG_FILE)) {
      return JSON.parse(readFileSync(LEGACY_CONFIG_FILE, 'utf8'));
    }
  } catch { }
  return {};
}
...
function createTempPath(prefix, extension = '') {
  ...
  return join(TMP_ROOT, `${prefix}-${process.pid}-${Date.now()}-${suffix}${extension}`);
}
...
function ensureDependencyInstalled(moduleName, installHint) {
  try {
    requireFromCli.resolve(moduleName);
  } catch {
    throw new Error(`Missing optional dependency "${moduleName}". Install it with: ${installHint}`);
  }
}
...
function figmaEvalSync(code) {
  ...
  if (daemonRunning) {
    const payloadFile = createTempPath('figma-payload', '.json');
    try {
      ...
      const curlArgs = ['-s', '-X', 'POST', `http://127.0.0.1:${DAEMON_PORT}/exec`, '-H', 'Content-Type: application/json'];
      if (daemonToken) curlArgs.push('-H', `X-Daemon-Token: ${daemonToken}`);
      curlArgs.push('-d', `@${payloadFile}`);
      const result = execFileSync('curl', curlArgs, { encoding: 'utf8', timeout: 60000 });
      ...
      cleanupFiles(payloadFile);
    }
  }

  // Fallback: direct connection via temp script
  const tempFile = createTempPath('figma-eval', '.mjs');
  const resultFile = createTempPath('figma-result', '.json');

  const script = `
    import { FigmaClient } from ${JSON.stringify(pathToFileURL(join(process.cwd(), 'src/figma-client.js')).href)};
  ...
  try {
    execFileSync('node', [tempFile], { stdio: 'pipe', timeout: 60000 });
    ...
    cleanupFiles(tempFile, resultFile);
  }
  return null;
}

// Helper: run code via fastEval and handle output for interactive commands
async function runEvalCommand(code, options = {}) {
  try {
...
program
  ...
  .action(async (url, options) => {
    ...
    const tempFile = createTempPath('figma-cli-screenshot', '.png');
...
program
  ...
  .action(async (url, options) => {
    ensureDependencyInstalled('playwright', 'npm install playwright');
    const spinner = ora('Analyzing ' + url + ' with Playwright...').start();
    const screenshotPath = options.screenshot ? createTempPath('figma-analyze-screenshot', '.png') : null;
...
(async () => {
  ...
  ${screenshotPath ? `await page.screenshot({ path: ${JSON.stringify(screenshotPath)} });` : ''}
  await browser.close();
})();
`;

      const result = execFileSync('node', ['--input-type=commonjs', '--eval', script], {
        cwd: process.cwd(),
        encoding: 'utf8',
        timeout: 90000,
...
`;
      ...
      const analysisResult = execFileSync('node', ['--input-type=commonjs', '--eval', analyzeScript], {
        cwd: process.cwd(),
        encoding: 'utf8',
        timeout: 90000,
        maxBuffer: 10 * 1024 * 1024
...
program
  ...
  .action(async (nodeId, options) => {
    ...
    const tempInput = createTempPath('figma-cli-removebg-input', '.png');
...
set
  ...
  .action(async (color, options) => {
    ...
    if (color.startsWith('var:')) {
      ...
      await runEvalCommand(codeStr, { silent: false });
    }
  });
...
program
  ...
  .action(async () => {
    ...
    try {
      const figmaUseLookup = process.platform === 'win32'
        ? 'where figma-use 2>nul'
        : 'command -v figma-use 2>/dev/null';
      execSync(figmaUseLookup, { encoding: 'utf8' });
